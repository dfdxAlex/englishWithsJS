...
export function extendAnalogZeroOneTwoFreeFour(array, strOld, strNew)
{
    return array.filter(el => el[0].toLowerCase().includes(strOld) 
            ...
            // то же самое для четвертой строки
            newEl[3] = newEl[3].replace(regexOne, searchStrNewOne);
            newEl[3] = newEl[3].replace(regexTwo, searchStrNewTwo);

            // то же самое для пятой строки
            newEl[4] = newEl[4].replace(regexOne, searchStrNewOne);
            newEl[4] = newEl[4].replace(regexTwo, searchStrNewTwo);
...
extendAnalogZeroOneTwoFreeFour.help = `
Функция расширяет массив заменяя слова синонимы
Функция просматривает массив на наличие в определенных предложениях заданных слов
Функция так-же создает дубликаты нужных слов с заглавными буквами и без и заменяет их.
Получается эффект замены как слова в начале предложения, так и в любом другом месте
