export const presentPerfectIHaveDone = [
    [
        "I have … my homework.", 
        "done", 
        "do", 
        "did", 
        "donee", 
        "Я сделал свою домашнюю работу.", 
        "Я зробив своє домашнє завдання.", 
        "Zrobiłem swoje zadanie domowe."
    ],
    [
        "She has … to the store.", 
        "gone", 
        "go", 
        "went", 
        "gonee", 
        "Она ушла в магазин.", 
        "Вона пішла до магазину.", 
        "Poszła do sklepu."
    ],
    [
        "Have they … the project?", 
        "finished", 
        "finish", 
        "finishe", 
        "finisheded", 
        "Они завершили проект?", 
        "Вони завершили проєкт?", 
        "Czy ukończyli projekt?"
    ],
    [
        "We have … this movie before.", 
        "seen", 
        "see", 
        "saw", 
        "seenn", 
        "Мы видели этот фильм раньше.", 
        "Ми бачили цей фільм раніше.", 
        "Widzieliśmy ten film wcześniej."
    ],
    [
        "He has not … the letter yet.", 
        "written", 
        "write", 
        "writed", 
        "wroten", 
        "Он еще не написал письмо.", 
        "Він ще не написав листа.", 
        "Nie napisał jeszcze listu."
    ],
    [
        "I have … this book.", 
        "read", 
        "readed", 
        "reed", 
        "rad", 
        "Я прочитал эту книгу.", 
        "Я прочитав цю книгу.", 
        "Przeczytałem tę książkę."
    ],
    [
        "She has … breakfast already.", 
        "eaten", 
        "eat", 
        "ate", 
        "eated", 
        "Она уже позавтракала.", 
        "Вона вже поснідала.", 
        "Ona już zjadła śniadanie."
    ],
    [
        "Have you … your car?", 
        "driven", 
        "drive", 
        "drove", 
        "drivenn", 
        "Вы водили свою машину?", 
        "Ви водили свою машину?", 
        "Czy prowadziłeś swój samochód?"
    ],
    [
        "We have … tickets for the concert.", 
        "bought", 
        "buy", 
        "buyed", 
        "boughten", 
        "Мы купили билеты на концерт.", 
        "Ми купили квитки на концерт.", 
        "Kupiliśmy bilety na koncert."
    ],
    [
        "He has just … the window.", 
        "opened", 
        "open", 
        "openned", 
        "openedd", 
        "Он только что открыл окно.", 
        "Він щойно відкрив вікно.", 
        "On właśnie otworzył okno."
    ],
    [
        "I have never … such a thing.", 
        "seen", 
        "see", 
        "saw", 
        "seenn", 
        "Я никогда не видел ничего подобного.", 
        "Я ніколи не бачив нічого подібного.", 
        "Nigdy czegoś takiego nie widziałem."
    ],
    [
        "Have they … the door?", 
        "closed", 
        "close", 
        "cloosed", 
        "closedd", 
        "Они закрыли дверь?", 
        "Вони закрили двері?", 
        "Czy zamknęli drzwi?"
    ],
    [
        "We have … a lot today.", 
        "worked", 
        "work", 
        "workked", 
        "workedd", 
        "Мы сегодня много работали.", 
        "Ми сьогодні багато працювали.", 
        "Dziś dużo pracowaliśmy."
    ],
    [
        "She has … the truth.", 
        "told", 
        "tell", 
        "telled", 
        "tolld", 
        "Она сказала правду.", 
        "Вона сказала правду.", 
        "Ona powiedziała prawdę."
    ],
    [
        "Have you … your homework?", 
        "done", 
        "do", 
        "did", 
        "donee", 
        "Вы сделали свою домашнюю работу?", 
        "Ви зробили своє домашнє завдання?", 
        "Czy zrobiłeś swoje zadanie domowe?"
    ],
    [
        "He has not … to us yet.", 
        "spoken", 
        "speak", 
        "spoke", 
        "spoked", 
        "Он еще не поговорил с нами.", 
        "Він ще не поговорив з нами.", 
        "Jeszcze z nami nie rozmawiał."
    ],
    [
        "They have … the cake.", 
        "baked", 
        "bake", 
        "bakedd", 
        "bakeed", 
        "Они испекли торт.", 
        "Вони спекли торт.", 
        "Upiekli ciasto."
    ],
    [
        "I have … my room.", 
        "cleaned", 
        "clean", 
        "cleanned", 
        "cleanedd", 
        "Я убрал свою комнату.", 
        "Я прибрав свою кімнату.", 
        "Posprzątałem swój pokój."
    ],
    [
        "Have you … your shoes?", 
        "polished", 
        "polish", 
        "polisshed", 
        "polisheded", 
        "Вы почистили свои ботинки?", 
        "Ви почистили свої черевики?", 
        "Czy wypolerowałeś swoje buty?"
    ],
    [
        "She has … the report already.", 
        "written", 
        "write", 
        "writed", 
        "wroten", 
        "Она уже написала отчет.", 
        "Вона вже написала звіт.", 
        "Ona już napisała raport."
    ],
    [
        "They have … a lot of money.", 
        "saved", 
        "save", 
        "savedd", 
        "saveed", 
        "Они сэкономили много денег.", 
        "Вони зекономили багато грошей.", 
        "Zaoszczędzili dużo pieniędzy."
    ],
    [
        "I have never … to Paris.", 
        "been", 
        "be", 
        "was", 
        "beenn", 
        "Я никогда не был в Париже.", 
        "Я ніколи не був у Парижі.", 
        "Nigdy nie byłem w Paryżu."
    ],
    [
        "She has … a new recipe.", 
        "tried", 
        "try", 
        "tryed", 
        "tryeed", 
        "Она попробовала новый рецепт.", 
        "Вона спробувала новий рецепт.", 
        "Spróbowała nowego przepisu."
    ],
    [
        "We have … to finish the task.", 
        "managed", 
        "manage", 
        "managged", 
        "manageed", 
        "Мы сумели завершить задачу.", 
        "Ми змогли завершити завдання.", 
        "Udało nam się ukończyć zadanie."
    ],
    [
        "He has … for the meeting.", 
        "prepared", 
        "prepare", 
        "preparred", 
        "prepareed", 
        "Он подготовился к встрече.", 
        "Він підготувався до зустрічі.", 
        "Przygotował się na spotkanie."
    ],
    [
        "They have not … the rules yet.", 
        "learned", 
        "learn", 
        "learntt", 
        "learnedd", 
        "Они еще не выучили правила.", 
        "Вони ще не вивчили правила.", 
        "Nie nauczyli się jeszcze zasad."
    ],
    [
        "Have you ever … in the ocean?", 
        "swum", 
        "swim", 
        "swam", 
        "swumm", 
        "Вы когда-нибудь плавали в океане?", 
        "Ви коли-небудь плавали в океані?", 
        "Czy kiedykolwiek pływałeś w oceanie?"
    ],
    [
        "She has just … the email.", 
        "sent", 
        "send", 
        "sended", 
        "sentt", 
        "Она только что отправила письмо.", 
        "Вона щойно відправила листа.", 
        "Właśnie wysłała e-mail."
    ],
    [
        "I have … my keys somewhere.", 
        "lost", 
        "lose", 
        "loost", 
        "lostt", 
        "Я потерял где-то свои ключи.", 
        "Я десь загубив свої ключі.", 
        "Zgubiłem gdzieś swoje klucze."
    ],
    [
        "We have … the match.", 
        "won", 
        "win", 
        "wonn", 
        "wined", 
        "Мы выиграли матч.", 
        "Ми виграли матч.", 
        "Wygraliśmy mecz."
    ],
    [
        "They have … the house.", 
        "painted", 
        "paint", 
        "paintted", 
        "paintedd", 
        "Они покрасили дом.", 
        "Вони пофарбували будинок.", 
        "Pomalowali dom."
    ],
    [
        "He has not … the movie yet.", 
        "watched", 
        "watch", 
        "watchhed", 
        "watchedd", 
        "Он еще не посмотрел фильм.", 
        "Він ще не подивився фільм.", 
        "Nie obejrzał jeszcze filmu."
    ],
    [
        "I have … him already.", 
        "called", 
        "call", 
        "calleed", 
        "calld", 
        "Я уже позвонил ему.", 
        "Я вже подзвонив йому.", 
        "Już do niego zadzwoniłem."
    ],
    [
        "We have … this problem many times.", 
        "discussed", 
        "discuss", 
        "discusseed", 
        "discussedd", 
        "Мы обсуждали эту проблему много раз.", 
        "Ми обговорювали цю проблему багато разів.", 
        "Omawialiśmy ten problem wiele razy."
    ],
    [
        "She has never … that song.", 
        "sung", 
        "sing", 
        "sang", 
        "sungg", 
        "Она никогда не пела эту песню.", 
        "Вона ніколи не співала цю пісню.", 
        "Nigdy nie zaśpiewała tej piosenki."
    ],
    [
        "Have you … this book?", 
        "read", 
        "readed", 
        "reed", 
        "rad", 
        "Вы читали эту книгу?", 
        "Ви читали цю книгу?", 
        "Czy przeczytałeś tę książkę?"
    ],
    [
        "He has … the tree in the garden.", 
        "planted", 
        "plant", 
        "plantted", 
        "plantedd", 
        "Он посадил дерево в саду.", 
        "Він посадив дерево в саду.", 
        "Zasadził drzewo w ogrodzie."
    ],
    [
        "They have … the housework.", 
        "finished", 
        "finish", 
        "finishe", 
        "finisheded", 
        "Они закончили работу по дому.", 
        "Вони закінчили роботу по дому.", 
        "Skończyli prace domowe."
    ],
    [
        "We have not … the answer yet.", 
        "found", 
        "find", 
        "finded", 
        "foundd", 
        "Мы еще не нашли ответ.", 
        "Ми ще не знайшли відповідь.", 
        "Nie znaleźliśmy jeszcze odpowiedzi."
    ],
    [
        "She has … the question twice.", 
        "asked", 
        "ask", 
        "askked", 
        "askedd", 
        "Она задала этот вопрос дважды.", 
        "Вона задала це питання двічі.", 
        "Zadała to pytanie dwa razy."
    ],
    [
        "Have you ever been to Paris?", 
        "Yes, I have been to Paris before.", 
        "I am in Paris now.", 
        "I will go to Paris next year.", 
        "I had been to Paris last year.", 
        "Вы когда-нибудь были в Париже?", 
        "Ви коли-небудь були в Парижі?", 
        "Czy kiedykolwiek byłeś w Paryżu?"
      ],
      [
        "Has she finished her homework?", 
        "No, she hasn't finished it yet.", 
        "She finishes her homework every day.", 
        "She will finish her homework tomorrow.", 
        "She had finished her homework yesterday.", 
        "Она уже закончила домашку?", 
        "Вона вже закінчила домашнє завдання?", 
        "Czy ona skończyła pracę domową?"
      ],
      [
        "Have you seen that movie?", 
        "Yes, I have seen it.", 
        "I watch it now.", 
        "I will watch it tomorrow.", 
        "I saw it last month.", 
        "Вы видели этот фильм?", 
        "Ви бачили цей фільм?", 
        "Czy widziałeś ten film?"
      ],
      [
        "Has he visited the new museum?", 
        "No, he hasn't had the chance to go yet.", 
        "He visits it every weekend.", 
        "He will visit it soon.", 
        "He visited it last week.", 
        "Он посетил новый музей?", 
        "Він відвідав новий музей?", 
        "Czy on odwiedził nowe muzeum?"
      ],
      [
        "Have they eaten lunch yet?", 
        "Yes, they ate already.", 
        "They eat lunch every day.", 
        "They will eat later.", 
        "They had lunch earlier.", 
        "Они уже поели?", 
        "Вони вже поїли?", 
        "Czy oni już jedli lunch?"
      ],
      [
        "Has it rained today?", 
        "No, it hasn't rained yet.", 
        "It rains every day.", 
        "It will rain later.", 
        "It rained yesterday.", 
        "Сегодня шел дождь?", 
        "Сьогодні йшов дощ?", 
        "Czy dzisiaj padał deszcz?"
      ],
      [
        "Have you completed your project?", 
        "Yes, I finished it yesterday.", 
        "I complete it every month.", 
        "I will complete it tomorrow.", 
        "I completed it last year.", 
        "Ты завершил проект?", 
        "Ти завершив проект?", 
        "Czy ukończyłeś swój projekt?"
      ],
      [
        "Has she called you today?", 
        "No, she hasn't called me yet.", 
        "She calls me every day.", 
        "She will call me later.", 
        "She called me yesterday.", 
        "Она звонила тебе сегодня?", 
        "Вона дзвонила тобі сьогодні?", 
        "Czy ona zadzwoniła do ciebie dzisiaj?"
      ],
      [
        "Have you read the book?", 
        "Yes, I have read the book already.", 
        "I am reading it right now.", 
        "I will read it tomorrow.", 
        "I read it last month.", 
        "Ты прочитал книгу?", 
        "Ти прочитав книгу?", 
        "Czy przeczytałeś książkę?"
      ],
      [
        "Has he finished the report?", 
        "Yes, he finished it this morning.", 
        "He finishes reports every week.", 
        "He will finish it tomorrow.", 
        "He had finished the report earlier.", 
        "Он закончил отчет?", 
        "Він закінчив звіт?", 
        "Czy on skończył raport?"
      ],
      [
        "Have they visited the new restaurant?", 
        "No, they haven't visited it yet.", 
        "They visit new places all the time.", 
        "They will visit it next week.", 
        "They visited it last weekend.", 
        "Они посетили новый ресторан?", 
        "Вони відвідали новий ресторан?", 
        "Czy oni odwiedzili nową restaurację?"
      ],
      [
        "Has it snowed this winter?", 
        "No, it hasn't snowed yet.", 
        "It snows every winter.", 
        "It will snow soon.", 
        "It snowed last year.", 
        "Зимой шел снег?", 
        "Цією зимою йшов сніг?", 
        "Czy w tym zimie padał śnieg?"
      ],
      [
        "Have you met her before?", 
        "Yes, I met her at the conference last year.", 
        "I meet her regularly.", 
        "I will meet her next week.", 
        "I met her yesterday.", 
        "Ты встречал её раньше?", 
        "Ти зустрічав її раніше?", 
        "Czy spotkałeś ją wcześniej?"
      ],
      [
        "Has he traveled abroad?", 
        "Yes, he has traveled to several countries.", 
        "He travels abroad every year.", 
        "He will travel soon.", 
        "He traveled last year.", 
        "Он путешествовал за границу?", 
        "Він подорожував за кордон?", 
        "Czy on podróżował za granicę?"
      ],
      [
        "Have they watched the match?", 
        "Yes, they watched it on TV last night.", 
        "They watch it every week.", 
        "They will watch it tomorrow.", 
        "They watched it last month.", 
        "Они посмотрели матч?", 
        "Вони подивилися матч?", 
        "Czy oni oglądali mecz?"
      ],
      [
        "Has she been to the doctor?", 
        "No, she hasn't been yet.", 
        "She goes to the doctor regularly.", 
        "She will go next week.", 
        "She went last month.", 
        "Она была у врача?", 
        "Вона була у лікаря?", 
        "Czy ona była u lekarza?"
      ],
      [
        "Have you spoken to him today?", 
        "Yes, I spoke to him this morning.", 
        "I speak to him every day.", 
        "I will speak to him later.", 
        "I spoke to him yesterday.", 
        "Ты говорил с ним сегодня?", 
        "Ти говорив з ним сьогодні?", 
        "Czy rozmawiałeś z nim dzisiaj?"
      ],
      [
        "Has he heard the news?", 
        "No, he hasn't heard it yet.", 
        "He hears the news every day.", 
        "He will hear it soon.", 
        "He heard it yesterday.", 
        "Он слышал новости?", 
        "Він чув новини?", 
        "Czy on słyszał wiadomości?"
      ],
      [
        "Have you found your keys?", 
        "Yes, I found them in my bag.", 
        "I find my keys every day.", 
        "I will find them later.", 
        "I found them last week.", 
        "Ты нашел свои ключи?", 
        "Ти знайшов свої ключі?", 
        "Czy znalazłeś swoje klucze?"
      ],
      [
        "Has she finished the painting?", 
        "No, she hasn't finished it yet.", 
        "She finishes paintings every month.", 
        "She will finish it soon.", 
        "She finished it last week.", 
        "Она закончила картину?", 
        "Вона закінчила картину?", 
        "Czy ona skończyła obraz?"
      ],
      [
        "Have they met each other?", 
        "Yes, they have met each other.", 
        "They meet regularly.", 
        "They will meet next month.", 
        "They met last year.", 
        "Они встретились?", 
        "Вони зустрілися?", 
        "Czy oni się spotkali?"
      ],
      [
        "Has it been raining all day?", 
        "No, it hasn't rained much today.", 
        "It rains every day.", 
        "It will rain later.", 
        "It rained all day yesterday.", 
        "Шел дождь весь день?", 
        "Йшов дощ увесь день?", 
        "Czy padał deszcz przez cały dzień?"
      ],
      [
        "Have you tried the new app?", 
        "Yes, I tried it yesterday.", 
        "I try new apps all the time.", 
        "I will try it tomorrow.", 
        "I tried it last week.", 
        "Ты пробовал новое приложение?", 
        "Ти пробував новий додаток?", 
        "Czy próbowałeś nowej aplikacji?"
      ],
      [
        "Has he fixed the car?", 
        "Yes, he fixed it this morning.", 
        "He fixes it every week.", 
        "He will fix it later.", 
        "He fixed it last week.", 
        "Он починил машину?", 
        "Він полагодив машину?", 
        "Czy on naprawił samochód?"
      ],
      [
        "Have they received the package?", 
        "Yes, they received it this morning.", 
        "They receive packages every week.", 
        "They will receive it tomorrow.", 
        "They received it last month.", 
        "Они получили посылку?", 
        "Вони отримали посилку?", 
        "Czy oni otrzymali paczkę?"
      ],
      [
        "Has it been a good day?", 
        "Yes, it has been a really good day.", 
        "It is a good day today.", 
        "It will be a good day tomorrow.", 
        "It was a good day yesterday.", 
        "Сегодня был хороший день?", 
        "Сьогодні був хороший день?", 
        "Czy dzisiaj był dobry dzień?"
      ],
      [
        "Yes, I have been to Paris before.", 
        "Have you ever been to Paris?", 
        "Are you in Paris now?", 
        "Will you go to Paris next year?", 
        "Had you been to Paris last year?", 
        "Да, я был в Париже раньше.", 
        "Так, я був у Парижі раніше.", 
        "Tak, byłem w Paryżu wcześniej."
      ],
      [
        "No, she hasn't finished it yet.", 
        "Has she finished her homework?", 
        "Does she finish it every day?", 
        "Will she finish her homework tomorrow?", 
        "Had she finished her homework yesterday?", 
        "Нет, она ещё не закончила.", 
        "Ні, вона ще не закінчила.", 
        "Nie, ona jeszcze tego nie skończyła."
      ],
      [
        "Yes, I watched it last weekend.", 
        "Have you seen that movie?", 
        "Are you watching it now?", 
        "Will you watch it tomorrow?", 
        "Did you watch it last month?", 
        "Да, я смотрел его на выходных.", 
        "Так, я дивився його минулого вихідного.", 
        "Tak, oglądałem go w zeszły weekend."
      ],
      [
        "No, he hasn't had the chance to go yet.", 
        "Has he had the chance to go?", 
        "Does he visit it every weekend?", 
        "Will he visit it soon?", 
        "Had he visited it last week?", 
        "Нет, у него ещё не было возможности.", 
        "Ні, в нього ще не було можливості.", 
        "Nie, nie miał jeszcze okazji go odwiedzić."
      ],
      [
        "Yes, they ate already.", 
        "Have they eaten lunch yet?", 
        "Do they eat lunch every day?", 
        "Will they eat later?", 
        "Had they eaten earlier?", 
        "Да, они уже поели.", 
        "Так, вони вже поїли.", 
        "Tak, już zjedli lunch."
      ],
      [
        "No, it hasn't snowed yet.", 
        "Has it snowed today?", 
        "Does it snow every day?", 
        "Will it snow later?", 
        "Did it snow last year?", 
        "Сегодня не было снега.", 
        "Сьогодні не було снігу.", 
        "Dzisiaj nie padał śnieg."
      ],
      [
        "Yes, I finished it yesterday.", 
        "Have you completed your project?", 
        "Do you complete it every month?", 
        "Will you complete it tomorrow?", 
        "Had you completed it last year?", 
        "Да, я закончил его вчера.", 
        "Так, я завершив його вчора.", 
        "Tak, skończyłem to wczoraj."
      ],
      [
        "No, she hasn't called me yet.", 
        "Has she called you today?", 
        "Does she call you every day?", 
        "Will she call you later?", 
        "Did she call you yesterday?", 
        "Она ещё не звонила.", 
        "Вона ще не дзвонила.", 
        "Jeszcze nie zadzwoniła."
      ],
      [
        "Yes, I have read the book already.", 
        "Have you read the book?", 
        "Are you reading it now?", 
        "Will you read it tomorrow?", 
        "Did you read it last month?", 
        "Да, я уже прочитал книгу.", 
        "Так, я вже прочитав книгу.", 
        "Tak, już przeczytałem książkę."
      ],
      [
        "Yes, he finished it this morning.", 
        "Has he finished the report?", 
        "Does he finish reports every week?", 
        "Will he finish it tomorrow?", 
        "Had he finished the report earlier?", 
        "Да, он закончил его сегодня утром.", 
        "Так, він закінчив його сьогодні вранці.", 
        "Tak, naprawił to dzisiaj rano."
      ],
      [
        "No, they haven't visited it yet.", 
        "Have they visited the new restaurant?", 
        "Do they visit new places all the time?", 
        "Will they visit it next week?", 
        "Did they visit it last weekend?", 
        "Они еще не посетили его.", 
        "Вони ще не відвідали його.", 
        "Nie odwiedzili jeszcze tej restauracji."
      ],
      [
        "No, it hasn't rained much today.", 
        "Has it been raining all day?", 
        "Does it rain every day?", 
        "Will it rain later?", 
        "Did it rain all day yesterday?", 
        "Сегодня не было много дождя.", 
        "Сьогодні не було багато дощу.", 
        "Dzisiaj nie padało dużo."
      ],
      [
        "Yes, I tried it yesterday.", 
        "Have you tried the new app?", 
        "Do you try new apps all the time?", 
        "Will you try it tomorrow?", 
        "Did you try it last week?", 
        "Да, я пробовал его вчера.", 
        "Так, я пробував його вчора.", 
        "Tak, próbowałem go wczoraj."
      ],
      [
        "Yes, he fixed it this morning.", 
        "Has he fixed the car?", 
        "Does he fix it every week?", 
        "Will he fix it later?", 
        "Did he fix it last week?", 
        "Да, он починил машину сегодня утром.", 
        "Так, він полагодив машину сьогодні вранці.", 
        "Tak, naprawił to dzisiaj rano."
      ],
      [
        "Yes, they received it this morning.", 
        "Have they received the package?", 
        "Do they receive packages every week?", 
        "Will they receive it tomorrow?", 
        "Did they receive it last month?", 
        "Да, они получили его сегодня утром.", 
        "Так, вони отримали його сьогодні вранці.", 
        "Tak, otrzymali to dzisiaj rano."
      ],
      [
        "Yes, it has been a really good day.", 
        "Has it been a good day?", 
        "Is it a good day today?", 
        "Will it be a good day tomorrow?", 
        "Was it a good day yesterday?", 
        "Сегодня был хороший день.", 
        "Сьогодні був хороший день.", 
        "Dzisiaj był dobry dzień."
      ],
      [
        "Yes, they watched it on TV last night.", 
        "Have they watched the match?", 
        "Do they watch it every week?", 
        "Will they watch it tomorrow?", 
        "Did they watch it yesterday?", 
        "Да, они смотрели матч по телевизору вчера вечером.", 
        "Так, вони дивились матч по телевізору вчора ввечері.", 
        "Tak, oglądali mecz wczoraj wieczorem."
      ],
      [
        "No, I haven't started yet.", 
        "Have you started your homework?", 
        "Did you start it yesterday?", 
        "Will you start it tomorrow?", 
        "Are you starting now?", 
        "Я еще не начал.", 
        "Я ще не почав.", 
        "Jeszcze nie zacząłem."
      ],
      [
        "Yes, she has traveled a lot this year.", 
        "Has she traveled a lot this year?", 
        "Did she travel last year?", 
        "Will she travel next year?", 
        "Has she been traveling often?", 
        "Она много путешествовала в этом году.", 
        "Вона багато подорожувала цього року.", 
        "Ona dużo podróżowała w tym roku."
      ],
      [
        "No, I haven't received it yet.", 
        "Have you received the letter?", 
        "Did you receive it last week?", 
        "Will you receive it tomorrow?", 
        "Had you received it earlier?", 
        "Я еще не получил.", 
        "Я ще не отримав.", 
        "Jeszcze nie otrzymałem."
      ],
      [
        "Have you finished your homework?",
        "Yes, I have just finished.",
        "Yes, I just have finished.",
        "Yes, I has just finished.",
        "Yes just, I have finished.",
        "Ты закончил свою домашнюю работу?",
        "Ти закінчив своє домашнє завдання?",
        "Czy skończyłeś swoje zadanie domowe?"
    ],
    [
        "Has she called her mother?",
        "Yes, I have just called.",
        "Yes, I just have called.",
        "Yes, I has just called.",
        "Yes just, I have called.",
        "Она позвонила своей маме?",
        "Вона подзвонила своїй мамі?",
        "Czy zadzwoniła do swojej mamy?"
    ],
    [
        "Have they eaten lunch?",
        "Yes, I have just eaten.",
        "Yes, I just have eaten.",
        "Yes, I has just eaten.",
        "Yes just, I have eaten.",
        "Они пообедали?",
        "Вони пообідали?",
        "Czy zjedli obiad?"
    ],
    [
        "Has he visited the museum?",
        "Yes, I have just visited.",
        "Yes, I just have visited.",
        "Yes, I has just visited.",
        "Yes just, I have visited.",
        "Он посетил музей?",
        "Він відвідав музей?",
        "Czy odwiedził muzeum?"
    ],
    [
        "Have we cleaned the house?",
        "Yes, I have just cleaned.",
        "Yes, I just have cleaned.",
        "Yes, I has just cleaned.",
        "Yes just, I have cleaned.",
        "Мы убрали дом?",
        "Ми прибрали дім?",
        "Czy posprzątaliśmy dom?"
    ],
    [
        "Have you watched the new movie?",
        "Yes, I have just watched.",
        "Yes, I just have watched.",
        "Yes, I has just watched.",
        "Yes just, I have watched.",
        "Ты посмотрел новый фильм?",
        "Ти подивився новий фільм?",
        "Czy obejrzałeś nowy film?"
    ],
    [
        "Has she completed the project?",
        "Yes, I have just completed.",
        "Yes, I just have completed.",
        "Yes, I has just completed.",
        "Yes just, I have completed.",
        "Она завершила проект?",
        "Вона завершила проект?",
        "Czy ukończyła projekt?"
    ],
    [
        "Have they read the book?",
        "Yes, I have just read.",
        "Yes, I just have read.",
        "Yes, I has just read.",
        "Yes just, I have read.",
        "Они прочитали книгу?",
        "Вони прочитали книгу?",
        "Czy przeczytali książkę?"
    ],
    [
        "Has he taken his medicine?",
        "Yes, I have just taken.",
        "Yes, I just have taken.",
        "Yes, I has just taken.",
        "Yes just, I have taken.",
        "Он принял свои лекарства?",
        "Він прийняв свої ліки?",
        "Czy wziął swoje lekarstwa?"
    ],
    [
        "Have you met the new teacher?",
        "Yes, I have just met.",
        "Yes, I just have met.",
        "Yes, I has just met.",
        "Yes just, I have met.",
        "Ты встретил нового учителя?",
        "Ти зустрів нового вчителя?",
        "Czy poznałeś nowego nauczyciela?"
    ],
    [
        "Has the train arrived?",
        "Yes, it has just arrived.",
        "Yes, I just have train.",
        "Yes, I has just train.",
        "Yes just, I have train.",
        "Поезд прибыл?",
        "Поїзд прибув?",
        "Czy pociąg przyjechał?"
    ],
    [
        "Have the kids gone to bed?",
        "Yes, I have just kids.",
        "Yes, I just have kids.",
        "Yes, I has just kids.",
        "Yes just, I have kids.",
        "Она позвонила своей маме?",
        "Вона подзвонила своїй мамі?",
        "Czy zadzwoniła do swojej mamy?"
    ],
    [
        "Has she signed the contract?",
        "Yes, she has just signed.",
        "Yes, she just has signed.",
        "Yes, i has just signed.",
        "Yes just, she has signed.",
        "Подписала ли она контракт?",
        "Вона підписала договір?",
        "Czy podpisała kontrakt?"
    ],
    [
        "Have they started the meeting?",
        "Yes, I have just started.",
        "Yes, I just have started.",
        "Yes, I has just started.",
        "Yes just, I have started.",
        "Он посетил музей?",
        "Він відвідав музей?",
        "Czy odwiedził muzeum?"
    ],
    [
        "Have you checked your email?",
        "Yes, I have just checked.",
        "Yes, I just have checked.",
        "Yes, I has just checked.",
        "Yes just, I have checked.",
        "Вы проверили свою электронную почту?",
        "Ви перевірили свою електронну пошту?",
        "Czy sprawdziłeś swoją pocztę e-mail?"
    ],
    [
        "Has he bought a new car?",
        "Yes, I have just bought.",
        "Yes, I just have bought.",
        "Yes, I has just bought.",
        "Yes just, I have bought.",
        "Ты посмотрел новый фильм?",
        "Ти подивився новий фільм?",
        "Czy obejrzałeś nowy film?"
    ],
    [
        "Have we packed our bags?",
        "Yes, we have just packed.",
        "Yes, we just have packed.",
        "Yes, we has just packed.",
        "Yes we just I have packed.",
        "Мы уже собрали чемоданы?",
        "Ми спакували валізи?",
        "Czy spakowaliśmy walizki?"
    ],
    [
        "Have you written the report?",
        "Yes, I have just written.",
        "Yes, I just have written.",
        "Yes, I has just written.",
        "Yes just, I have written.",
        "Они прочитали книгу?",
        "Вони прочитали книгу?",
        "Czy przeczytali książkę?"
    ],
    [
        "Has she prepared dinner?",
        "Yes, I have just prepared.",
        "Yes, I just have prepared.",
        "Yes, I has just prepared.",
        "Yes just, I have prepared.",
        "Он принял свои лекарства?",
        "Він прийняв свої ліки?",
        "Czy wziął swoje lekarstwa?"
    ],
    [
        "Have they joined the team?",
        "Yes, I have just joined.",
        "Yes, I just have joined.",
        "Yes, I has just joined.",
        "Yes just, I have joined.",
        "Ты встретил нового учителя?",
        "Ти зустрів нового вчителя?",
        "Czy poznałeś nowego nauczyciela?"
    ],
    [
        "Have you closed the window?",
        "Yes, I have just closed.",
        "Yes, I just have closed.",
        "Yes, I has just closed.",
        "Yes just, I have closed.",
        "Ты закончил свою домашнюю работу?",
        "Ти закінчив своє домашнє завдання?",
        "Czy skończyłeś swoje zadanie domowe?"
    ],
    [
        "Has he repaired the bike?",
        "Yes, I have just repaired.",
        "Yes, I just have repaired.",
        "Yes, I has just repaired.",
        "Yes just, I have repaired.",
        "Она позвонила своей маме?",
        "Вона подзвонила своїй мамі?",
        "Czy zadzwoniła do swojej mamy?"
    ],
    [
        "Have we planted the flowers?",
        "Yes, we have just planted.",
        "Yes, we just have planted.",
        "Yes, we has just planted.",
        "Yes just, I have planted.",
        "Да, мы только что посадили.",
        "Так, ми щойно посадили.",
        "Tak, właśnie zasadziliśmy."
    ],
    [
        "Have you organized the files?",
        "Yes, I have just organized the files.",
        "Yes, I just have organized the files.",
        "Yes, I has just organized the files.",
        "Yes just, I have organized the files.",
        "Вы организовали файлы?",
        "Ви впорядкували файли?",
        "Czy uporządkowałeś pliki?"
    ],
    [
        "Has she painted the picture?",
        "Yes, I have just painted.",
        "Yes, I just have painted.",
        "Yes, I has just painted.",
        "Yes just, I have painted.",
        "Мы убрали дом?",
        "Ми прибрали дім?",
        "Czy posprzątaliśmy dom?"
    ],
    [
        "Have they fixed the computer?",
        "Yes, I have just fixed.",
        "Yes, I just have fixed.",
        "Yes, I has just fixed.",
        "Yes just, I have fixed.",
        "Ты посмотрел новый фильм?",
        "Вони полагодили комп’ютер?",
        "Они починили компьютер?"
    ],
    [
        "Has he opened the shop?",
        "Yes, I have just opened.",
        "Yes, I just have opened.",
        "Yes, I has just opened.",
        "Yes just, I have opened.",
        "Она завершила проект?",
        "Вона завершила проект?",
        "Czy ukończyła projekt?"
    ],
    [
        "Have you watered the plants?",
        "Yes, I have just watered.",
        "Yes, I just have watered.",
        "Yes, I has just watered.",
        "Yes just, I have watered.",
        "Вы полили растения?",
        "Ви полили рослини?",
        "Czy podlewałeś rośliny?"
    ],
    [
        "Has she learned the song?",
        "Yes, I have just learned.",
        "Yes, I just have learned.",
        "Yes, I has just learned.",
        "Yes just, I have learned.",
        "Она выучила песню?",
        "Вона вивчила пісню?",
        "Czy ona nauczyła się tej piosenki?"
    ],
    [
        "Have you planned the trip?",
        "Yes, I have just planned the trip.",
        "Yes, I just have planned the trip.",
        "Yes, I has just planned the trip.",
        "Yes just, I have planned the trip.",
        "Они запланировали поездку?",
        "Вони запланували поїздку?",
        "Czy zaplanowali tę podróż?"
    ],
    [
        "Have you called the plumber?",
        "Yes, I have just called the plumber?.",
        "Yes, I just have called the plumber?.",
        "Yes, I has just called the plumber?.",
        "Yes just, I have called the plumber?.",
        "Вы вызывали сантехника?",
        "Ви викликали сантехніка?",
        "Czy dzwoniłeś po hydraulika?"
    ],
    [
        "Has he answered the question?",
        "Yes, he has just answered.",
        "Yes, he just has answered.",
        "Yes, we has just answered.",
        "Yes just, he has answered.",
        "Ответил ли он на вопрос?",
        "Він відповів на запитання?",
        "Czy odpowiedział na pytanie?"
    ],
    [
        "Have we solved the problem?",
        "Yes, we have just solved the problem.",
        "Yes, we just have solved the problem..",
        "Yes, we has just solved the problem..",
        "Yes just, we have solved the problem..",
        "Решили ли мы проблему?",
        "Ми вирішили проблему?",
        "Czy rozwiązaliśmy problem?"
    ],
    [
        "Have you fed the dog?",
        "Yes, I have just fed.",
        "Yes, I just have fed.",
        "Yes, I has just fed.",
        "Yes just, I have fed.",
        "Он посетил музей?",
        "Він відвідав музей?",
        "Czy odwiedził muzeum?"
    ],
    [
        "Has she arranged the meeting?",
        "Yes, I have just arranged the meeting.",
        "Yes, I just have arranged the meeting.",
        "Yes, I has just arranged the meeting.",
        "Yes just, I have arranged the meeting.",
        "Она организовала встречу?",
        "Вона домовилася про зустріч?",
        "Czy ona zorganizowała spotkanie?"
    ],
    [
        "Have they discussed the issue?",
        "Yes, they have just discussed.",
        "Yes, they just have discussed.",
        "Yes, they has just discussed.",
        "Yes just, they have discussed.",
        "Обсуждали ли они этот вопрос?",
        "Вони обговорювали це питання?",
        "Czy omówili ten problem?"
    ],
    [
        "Has he decorated the room?",
        "Yes, I have just decorated the room.",
        "Yes, I just have decorated the room.",
        "Yes, I has just decorated the room.",
        "Yes just, I have decorated the room.",
        "Он украсил комнату?",
        "Він прикрасив кімнату?",
        "Czy on udekorował pokój?"
    ],
    [
        "Have you signed the papers?",
        "Yes, I have just signed.",
        "Yes, I just have signed.",
        "Yes, I has just signed.",
        "Yes just, I have signed.",
        "Вы подписали документы?",
        "Ви підписали папери?",
        "Czy podpisałeś papiery?"
    ],
    [
        "Has she explained the task?",
        "Yes, she has just explained.",
        "Yes, I just have explained.",
        "Yes, I has just explained.",
        "Yes just, I have explained.",
        "Объяснила ли она задачу?",
        "Вона пояснила завдання?",
        "Czy wyjaśniła zadanie?"
    ],
    [
        "Have they finished the homework?",
        "Yes, they have just finished.",
        "Yes, they just have finished.",
        "Yes, they has just finished.",
        "Yes just, they have finished.",
        "Они закончили домашнее задание?",
        "Вони закінчили домашнє завдання?",
        "Czy skończyli pracę domową?"
    ],
    [
      "Have you already eaten?",
      "Yes, I have already eaten.",
      "Yes, already I have eaten.",
      "I have eaten already.",
      "Already I have eaten.",
      "Вы уже поели?",
      "Ви вже поїли?",
      "Czy już jedliście?"
  ],
  [
      "Has he already finished his homework?",
      "Yes, he has already finished it.",
      "Yes, he finished already.",
      "Already he has finished it.",
      "He already has finished.",
      "Он уже закончил домашку?",
      "Він вже закінчив домашнє завдання?",
      "On już skończył swoją pracę domową?"
  ],
  [
      "Have they already arrived?",
      "Yes, they have already arrived.",
      "They have already arrived.",
      "Arrived they already have.",
      "Already arrived they have.",
      "Они уже приехали?",
      "Вони вже приїхали?",
      "Oni już przyjechali?"
  ],
  [
      "Have we already met?",
      "Yes, we have already met.",
      "We have met already.",
      "Met we already have?",
      "Already have we met?",
      "Мы уже встретились?",
      "Ми вже зустрічались?",
      "My już się spotkaliśmy?"
  ],
  [
      "Has she already left?",
      "Yes, she has already left.",
      "She already has left.",
      "Already she has left.",
      "She has left already.",
      "Она уже ушла?",
      "Вона вже пішла?",
      "Ona już wyszła?"
  ],
  [
      "Has he already seen the movie?",
      "Yes, he has already seen it.",
      "He already has seen the movie.",
      "Already he has seen it.",
      "Seen he already has the movie.",
      "Он уже видел этот фильм?",
      "Він вже бачив цей фільм?",
      "On już widział ten film?"
  ],
  [
      "Have they already started the meeting?",
      "Yes, they have already started.",
      "They already have started.",
      "Already they started have.",
      "Started they already have.",
      "Они уже начали встречу?",
      "Вони вже почали зустріч?",
      "Oni już zaczęli spotkanie?"
  ],
  [
      "Has she already told you?",
      "Yes, she has already told me.",
      "She already has told me.",
      "Told she already has me.",
      "Already has she told me.",
      "Она уже рассказала тебе?",
      "Вона вже розповіла тобі?",
      "Ona już ci powiedziała?"
  ],
  [
      "Have you already seen the new album?",
      "Yes, I have already seen it.",
      "I already have seen the new album.",
      "Seen I already have the new album.",
      "Already I have seen the new album.",
      "Ты уже видел новый альбом?",
      "Ти вже бачив новий альбом?",
      "Czy już widziałeś nowy album?"
  ],
  [
      "Have they already made a decision?",
      "Yes, they have already made it.",
      "They made it already.",
      "Already made they it.",
      "They have it made already.",
      "Они уже приняли решение?",
      "Вони вже прийняли рішення?",
      "Oni już podjęli decyzję?"
  ],
  [
      "Have we already paid?",
      "Yes, we have already paid.",
      "We paid already.",
      "Paid we already have.",
      "Already we paid.",
      "Мы уже заплатили?",
      "Ми вже заплатили?",
      "Czy już zapłaciliśmy?"
  ],
  [
      "Has he already bought a new phone?",
      "Yes, he has already bought it.",
      "He bought already a new phone.",
      "Already bought he it.",
      "Bought he already a new phone.",
      "Он уже купил новый телефон?",
      "Він вже купив новий телефон?",
      "On już kupił nowy telefon?"
  ],
  [
      "Have they already visited the museum?",
      "Yes, they have already visited it.",
      "They have already visited the museum.",
      "Visited they already the museum.",
      "Already they visited the museum.",
      "Они уже посетили музей?",
      "Вони вже відвідали музей?",
      "Oni już odwiedzili muzeum?"
  ],
  [
      "Has she already received the package?",
      "Yes, she has already received it.",
      "She has received already the package.",
      "Already received she has the package.",
      "Received she already has the package.",
      "Она уже получила посылку?",
      "Вона вже отримала посилку?",
      "Ona już otrzymała paczkę?"
  ],
  [
      "Have we already made an agreement?",
      "Yes, we have already made it.",
      "We made it already.",
      "Already made we it.",
      "Made we already have it.",
      "Мы уже заключили соглашение?",
      "Ми вже уклали угоду?",
      "Czy już zawarliśmy umowę?"
  ],
  [
      "Has he already left the office?",
      "Yes, he has already left.",
      "He already has left.",
      "Already he has left.",
      "He has already left the office.",
      "Он уже ушел из офиса?",
      "Він вже пішов з офісу?",
      "On już opuścił biuro?"
  ],
  [
      "Have they already written the report?",
      "Yes, they have already written it.",
      "They have written it already.",
      "Written they have already the report.",
      "Already they have written the report.",
      "Они уже написали отчет?",
      "Вони вже написали звіт?",
      "Oni już napisali raport?"
  ],
  [
      "Has she already called you?",
      "Yes, she has already called me.",
      "She has called me already.",
      "Already has she called me?",
      "She already has called me.",
      "Она уже позвонила тебе?",
      "Вона вже подзвонила тобі?",
      "Ona już zadzwoniła do ciebie?"
  ],
  [
      "Have you already finished your project?",
      "Yes, I have already finished it.",
      "I finished it already.",
      "Already I have finished it.",
      "Finished I already have.",
      "Ты уже закончил проект?",
      "Ти вже закінчив проект?",
      "Czy już skończyłeś swój projekt?"
  ],
  [
      "Has he already gone home?",
      "Yes, he has already gone.",
      "He already has gone home.",
      "Already he gone has home.",
      "Gone he has already home.",
      "Он уже ушел домой?",
      "Він вже пішов додому?",
      "On już poszedł do domu?"
  ],
  [
      "Have they already prepared everything?",
      "Yes, they have already prepared it.",
      "They have already prepared everything.",
      "Prepared they already have everything.",
      "Already prepared they have everything.",
      "Они уже подготовили все?",
      "Вони вже підготували все?",
      "Oni już przygotowali wszystko?"
  ],
  [
      "Has she already sent the email?",
      "Yes, she has already sent it.",
      "She already has sent the email.",
      "Sent already she has the email.",
      "Already the email has sent she.",
      "Она уже отправила письмо?",
      "Вона вже надіслала лист?",
      "Ona już wysłała e-mail?"
  ],
  [
      "Have we already eaten dinner?",
      "Yes, we have already eaten.",
      "We already have eaten dinner.",
      "Eaten we already have dinner.",
      "Already we eaten have dinner.",
      "Мы уже поужинали?",
      "Ми вже поїли вечерю?",
      "Czy już jedliśmy kolację?"
  ],
  [
      "Have they already seen the new show?",
      "Yes, they have already seen it.",
      "They have already seen the new show.",
      "Already they seen have the new show.",
      "Seen they already have the new show.",
      "Они уже видели новое шоу?",
      "Вони вже бачили нове шоу?",
      "Oni już widzieli nowy program?"
  ],
  [
      "Has she already checked her email?",
      "Yes, she has already checked it.",
      "She already has checked her email.",
      "Checked already she has her email.",
      "Already checked she has her email.",
      "Она уже проверила свою почту?",
      "Вона вже перевірила свою пошту?",
      "Ona już sprawdziła swój e-mail?"
  ],
  [
      "Have we already agreed on the terms?",
      "Yes, we have already agreed.",
      "We have already agreed on the terms.",
      "Agreed we already have on the terms.",
      "Already we have agreed on the terms.",
      "Мы уже согласовали условия?",
      "Ми вже погодили умови?",
      "Czy już uzgodniliśmy warunki?"
  ],
  [
      "Has he already paid the bill?",
      "Yes, he has already paid it.",
      "He already has paid the bill.",
      "Already he paid has the bill.",
      "Paid he already has the bill.",
      "Он уже оплатил счет?",
      "Він вже оплатив рахунок?",
      "On już zapłacił rachunek?"
  ],
  [
      "Have they already written the letter?",
      "Yes, they have already written it.",
      "They already have written the letter.",
      "Written already they have the letter.",
      "Already written they have the letter.",
      "Они уже написали письмо?",
      "Вони вже написали лист?",
      "Oni już napisali list?"
  ],
  [
      "Has she already received the invitation?",
      "Yes, she has already received it.",
      "She already has received the invitation.",
      "Received she already has the invitation.",
      "Already has she received the invitation.",
      "Она уже получила приглашение?",
      "Вона вже отримала запрошення?",
      "Ona już otrzymała zaproszenie?"
  ],
  [
      "Have we already discussed this matter?",
      "Yes, we have already discussed it.",
      "We already discussed it already.",
      "Discussed we already have it.",
      "Already discussed we have it.",
      "Мы уже обсудили этот вопрос?",
      "Ми вже обговорили це питання?",
      "Czy już omówiliśmy tę sprawę?"
  ],
  [
      "Has he already gone to bed?",
      "Yes, he has already gone.",
      "He already has gone to bed.",
      "Already he gone has to bed.",
      "Gone he already has to bed.",
      "Он уже лег спать?",
      "Він вже ляг спати?",
      "On już poszedł do łóżka?"
  ],
  [
      "Have they already moved to the new house?",
      "Yes, they have already moved.",
      "They already have moved to the new house.",
      "Already they moved have to the new house.",
      "Moved they already have to the new house.",
      "Они уже переехали в новый дом?",
      "Вони вже переїхали в новий будинок?",
      "Oni już przeprowadzili się do nowego domu?"
  ],
  [
      "Has she already finished the book?",
      "Yes, she has already finished it.",
      "She already finished it.",
      "Finished already she has it.",
      "Already finished she has it.",
      "Она уже закончила книгу?",
      "Вона вже закінчила книгу?",
      "Ona już skończyła książkę?"
  ],
  [
      "Have you already heard the news?",
      "Yes, I have already heard it.",
      "I already have heard the news.",
      "Heard I already have the news.",
      "Already I have heard the news.",
      "Ты уже слышал новости?",
      "Ти вже чув новини?",
      "Czy już słyszałeś wiadomości?"
  ],
  [
      "Has he already done his work?",
      "Yes, he has already done it.",
      "He done already has his work.",
      "Done he already has his work.",
      "Already done he has his work.",
      "Он уже сделал свою работу?",
      "Він вже зробив свою роботу?",
      "On już wykonał swoją pracę?"
  ],
  [
    "Have you eaten yet?",
    "Yes, I have just eaten.",
    "Yes, I haven't eaten yet.",
    "I have yet eaten.",
    "I haven't just eat yet.",
    "Вы уже поели?",
    "Ви вже поїли?",
    "Czy już zjadłeś?"
],
[
    "Has he finished yet?",
    "No, he hasn't finished yet.",
    "Yes, he hasn't finished yet.",
    "He has yet finished.",
    "He finished yet.",
    "Он уже закончил?",
    "Він вже закінчив?",
    "Czy on już skończył?"
],
[
    "Has she called you yet?",
    "No, she hasn't called me yet.",
    "Yes, she has just called me.",
    "She hasn't called yet.",
    "She called yet.",
    "Она уже позвонила?",
    "Вона вже подзвонила?",
    "Czy ona już zadzwoniła?"
],
[
    "Have we reached the station yet?",
    "No, we haven't reached the station yet.",
    "Yes, we have reached the station.",
    "We haven't reached the station yet.",
    "We reached the station yet.",
    "Мы уже приехали на станцию?",
    "Ми вже досягли станції?",
    "Czy już dotarliśmy na stację?"
],
[
    "Have they arrived yet?",
    "Yes, they have just arrived.",
    "No, they haven't arrived yet.",
    "They have arrived yet.",
    "They haven't just arrived yet.",
    "Они уже прибыли?",
    "Вони вже прибули?",
    "Czy oni już przyjechali?"
],
[
    "Has he returned yet?",
    "Yes, he has already returned.",
    "No, he has not yet returned.",
    "He has returned yet.",
    "He has not returned yet.",
    "Он уже вернулся?",
    "Він вже повернувся?",
    "Czy on już wrócił?"
],
[
    "Have you watched the new movie yet?",
    "Yes, I have just watched it.",
    "No, I have not watched it yet.",
    "I haven't watched the movie yet.",
    "I have watched it yet.",
    "Ты уже смотрел новый фильм?",
    "Ти вже подивився новий фільм?",
    "Czy już obejrzałeś nowy film?"
],
[
    "Has she been to the new café yet?",
    "Yes, she has already been there.",
    "No, she hasn't been to the café yet.",
    "She has been to the café yet.",
    "She hasn't been yet.",
    "Она уже была в новом кафе?",
    "Вона вже була в новому кафе?",
    "Czy ona była już w nowej kawiarni?"
],
[
    "Have you talked to John yet?",
    "No, I haven't talked to him yet.",
    "Yes, I have just talked to John.",
    "I have talked to John yet.",
    "I haven't talked yet.",
    "Ты уже поговорил с Джоном?",
    "Ти вже поговорив з Джоном?",
    "Czy już rozmawiałeś z Johnem?"
],
[
    "Has he eaten lunch yet?",
    "No, he hasn't eaten lunch yet.",
    "Yes, he has eaten lunch yet.",
    "He has eaten lunch yet.",
    "He hasn't lunch eaten yet.",
    "Он уже поел?",
    "Він вже поїв?",
    "Czy on już zjadł obiad?"
],
[
    "Have they started the meeting yet?",
    "Yes, they have already started the meeting.",
    "No, they haven't started the meeting yet.",
    "They haven't started yet.",
    "They have started yet.",
    "Они уже начали собрание?",
    "Вони вже почали зустріч?",
    "Czy oni już zaczęli spotkanie?"
],
[
    "Have you finished your homework yet?",
    "Yes, I have just finished it.",
    "No, I haven't finished it yet.",
    "I have finished yet.",
    "I haven't finished homework yet.",
    "Ты уже закончил домашку?",
    "Ти вже закінчив домашнє завдання?",
    "Czy już skończyłeś pracę domową?"
],
[
    "Has he spoken to the teacher yet?",
    "No, he hasn't spoken to the teacher yet.",
    "Yes, he has just spoken to the teacher.",
    "He hasn't spoken yet.",
    "He spoken yet.",
    "Он уже поговорил с учителем?",
    "Він вже поговорив з учителем?",
    "Czy on już rozmawiał z nauczycielem?"
],
[
    "Have they left yet?",
    "Yes, they have already left.",
    "No, they haven't left yet.",
    "They have left yet.",
    "They haven't yet left.",
    "Они уже ушли?",
    "Вони вже пішли?",
    "Czy oni już wyszli?"
],
[
    "Has she decided yet?",
    "Yes, she has already decided.",
    "No, she hasn't decided yet.",
    "She has decided yet.",
    "She hasn't decided yet.",
    "Она уже решила?",
    "Вона вже вирішила?",
    "Czy ona już zdecydowała?"
],
[
    "Have you met my friend yet?",
    "Yes, I have just met him.",
    "No, I haven't met your friend yet.",
    "I have met your friend yet.",
    "I haven't met yet.",
    "Ты уже встретил моего друга?",
    "Ти вже зустрів мого друга?",
    "Czy już spotkałeś mojego przyjaciela?"
],
[
    "Has he played the game yet?",
    "No, he hasn't played it yet.",
    "Yes, he have just played it.",
    "He played it yet.",
    "He hasn't just played it yet.",
    "Он уже сыграл в игру?",
    "Він вже грав у гру?",
    "Czy on już grał w grę?"
],
[
    "Have they written the report yet?",
    "No, they haven't written it yet.",
    "Yes, they have already written the report.",
    "They have written the report yet.",
    "They haven't written yet.",
    "Они уже написали отчет?",
    "Вони вже написали звіт?",
    "Czy oni już napisali raport?"
],
[
    "Have you called the doctor yet?",
    "Yes, I have just called the doctor.",
    "No, I haven't called him yet.",
    "I have called the doctor yet.",
    "I haven't called doctor yet.",
    "Ты уже звонил врачу?",
    "Ти вже дзвонив лікарю?",
    "Czy już zadzwoniłeś do doktora?"
],
[
    "Has he checked his email yet?",
    "Yes, he has just checked it.",
    "No, he hasn't checked his email yet.",
    "He has checked email yet.",
    "He hasn't checked yet.",
    "Он уже проверил свою почту?",
    "Він вже перевірив свою пошту?",
    "Czy on już sprawdził swój e-mail?"
],
[
    "Have they made a decision yet?",
    "Yes, they have already made a decision.",
    "No, they haven't made a decision yet.",
    "They have made a decision yet.",
    "They haven't yet made a decision.",
    "Они уже приняли решение?",
    "Вони вже прийняли рішення?",
    "Czy oni już podjęli decyzję?"
],
[
    "Have you read the book yet?",
    "Yes, I have already read it.",
    "No, I haven't read it yet.",
    "I haven't read yet.",
    "I read the book yet.",
    "Ты уже прочитал книгу?",
    "Ти вже прочитав книгу?",
    "Czy już przeczytałeś książkę?"
],
[
    "Has she answered the question yet?",
    "No, she hasn't answered yet.",
    "Yes, she has answered it already.",
    "She has answered the question yet.",
    "She hasn't yet answered.",
    "Она уже ответила на вопрос?",
    "Вона вже відповіла на питання?",
    "Czy ona już odpowiedziała na pytanie?"
],
[
    "Have we seen the movie yet?",
    "No, we haven't seen it yet.",
    "Yes, we have already seen it.",
    "We have seen the movie yet.",
    "We haven't seen yet.",
    "Мы уже видели фильм?",
    "Ми вже бачили фільм?",
    "Czy już widzieliśmy film?"
],
[
    "Has he asked for help yet?",
    "No, he hasn't asked yet.",
    "Yes, he has just asked for help.",
    "He has asked for help yet.",
    "He hasn't asked yet.",
    "Он уже попросил помощи?",
    "Він вже попросив допомоги?",
    "Czy on już poprosił o pomoc?"
],
[
    "Have you worked on the project yet?",
    "Yes, I have just worked on it.",
    "No, I haven't worked on it yet.",
    "I have worked on it yet.",
    "I haven't worked yet.",
    "Ты уже работал над проектом?",
    "Ти вже працював над проектом?",
    "Czy już pracowałeś nad projektem?"
],
[
    "Has she cleaned the house yet?",
    "Yes, she has already cleaned it.",
    "No, she hasn't cleaned it yet.",
    "She has cleaned the house yet.",
    "She hasn't cleaned yet.",
    "Она уже убрала дом?",
    "Вона вже прибрала будинок?",
    "Czy ona już posprzątała dom?"
],
[
    "Have they understood the lesson yet?",
    "Yes, they have just understood it.",
    "No, they haven't understood the lesson yet.",
    "They have understood yet.",
    "They haven't understood yet.",
    "Они уже поняли урок?",
    "Вони вже зрозуміли урок?",
    "Czy oni już zrozumieli lekcję?"
],
[
    "Has he written the letter yet?",
    "Yes, he has already written it.",
    "No, he hasn't written the letter yet.",
    "He has written it yet.",
    "He hasn't written yet.",
    "Он уже написал письмо?",
    "Він вже написав лист?",
    "Czy on już napisał list?"
],
[
    "Have you done the work yet?",
    "Yes, I have just done it.",
    "No, I haven't done the work yet.",
    "I have done it yet.",
    "I haven't done yet.",
    "Ты уже сделал работу?",
    "Ти вже зробив роботу?",
    "Czy już zrobiłeś pracę?"
],
[
    "Has he bought the tickets yet?",
    "Yes, he has already bought them.",
    "No, he hasn't bought the tickets yet.",
    "He has bought tickets yet.",
    "He hasn't yet bought the tickets.",
    "Он уже купил билеты?",
    "Він вже купив квитки?",
    "Czy on już kupił bilety?"
],
[
    "Have they helped you yet?",
    "Yes, they have already helped me.",
    "No, they haven't helped me yet.",
    "They have helped yet.",
    "They haven't helped yet.",
    "Они уже помогли тебе?",
    "Вони вже допомогли тобі?",
    "Czy oni już ci pomogli?"
],
[
    "Has she come yet?",
    "Yes, she has just come.",
    "She have not come yet.",
    "She has come yet.",
    "She come hasn't yet.",
    "Она уже пришла?",
    "Вона вже прийшла?",
    "Czy ona już przyszła?"
],
[
    "Have you finished reading yet?",
    "Yes, I have already finished reading.",
    "No, I haven't finished reading yet.",
    "I have finished reading yet.",
    "I haven't finished yet.",
    "Ты уже закончил читать?",
    "Ти вже закінчив читати?",
    "Czy już skończyłeś czytać?"
],
[
    "Has he played the guitar yet?",
    "Yes, he has just played it.",
    "No, he hasn't played the guitar yet.",
    "He has played it yet.",
    "He hasn't yet played it.",
    "Он уже играл на гитаре?",
    "Він вже грав на гітарі?",
    "Czy on już grał na gitarze?"
],
[
    "Have you ever been to Paris?",
    "Yes, I have been to Paris.",
    "No, I been to Paris.",
    "Yes, I have to Paris.",
    "I never have been to Paris.",
    "Ты когда-нибудь был в Париже?",
    "Ти коли-небудь був у Парижі?",
    "Czy kiedykolwiek byłeś w Paryżu?"
  ],
  [
    "Has she never eaten sushi?",
    "No, she has never eaten sushi.",
    "Yes, she have never eaten sushi.",
    "No, she has eaten sushi.",
    "She has never eat sushi.",
    "Она никогда не ела суши?",
    "Вона ніколи не їла суші?",
    "Czy ona nigdy nie jadła sushi?"
  ],
  [
    "Have they ever played tennis?",
    "Yes, they have played tennis.",
    "No, they has played tennis.",
    "Yes, they ever played tennis.",
    "They have ever played tennis?",
    "Они когда-нибудь играли в теннис?",
    "Вони коли-небудь грали в теніс?",
    "Czy oni kiedykolwiek grali w tenisa?"
  ],
  [
    "Have we never tried this game before?",
    "No, we have never tried this game before.",
    "Yes, we has never tried this game.",
    "No, we have tried this game.",
    "We have never try this game before.",
    "Мы никогда не пробовали эту игру раньше?",
    "Ми ніколи не пробували цю гру раніше?",
    "Czy nigdy wcześniej nie próbowaliśmy tej gry?"
  ],
  [
    "Has he ever met a famous person?",
    "Yes, he has met a famous person.",
    "No, he never met a famous person.",
    "Yes, he has meet a famous person.",
    "He has ever met a famous person?",
    "Он когда-нибудь встречал знаменитость?",
    "Він коли-небудь зустрічав відому людину?",
    "Czy on kiedykolwiek spotkał sławną osobę?"
  ],
  [
    "Have you never seen this movie?",
    "No, I have never seen this movie.",
    "Yes, I has never seen this movie.",
    "No, I have saw this movie.",
    "I never have seen this movie.",
    "Ты никогда не видел этот фильм?",
    "Ти ніколи не бачив цей фільм?",
    "Czy nigdy nie widziałeś tego filmu?"
  ],
  [
    "Have they ever traveled by plane?",
    "Yes, they have traveled by plane.",
    "No, they has traveled by plane.",
    "Yes, they have traveling by plane.",
    "They have ever traveled by plane.",
    "Они когда-нибудь летали на самолёте?",
    "Вони коли-небудь літали літаком?",
    "Czy oni kiedykolwiek podróżowali samolotem?"
  ],
  [
    "Have you never visited this museum?",
    "No, I have never visited this museum.",
    "Yes, I has visited this museum.",
    "No, I have visit this museum.",
    "I never have visited this museum.",
    "Ты никогда не посещал этот музей?",
    "Ти ніколи не відвідував цей музей?",
    "Czy nigdy nie odwiedziłeś tego muzeum?"
  ],
  [
    "Has she ever worked abroad?",
    "Yes, she has worked abroad.",
    "No, she has work abroad.",
    "Yes, she has ever worked abroad.",
    "She has ever worked abroad?",
    "Она когда-нибудь работала за границей?",
    "Вона коли-небудь працювала за кордоном?",
    "Czy ona kiedykolwiek pracowała za granicą?"
  ],
  [
    "Have we never talked about this topic?",
    "No, we have never talked about this topic.",
    "Yes, we has talked about this topic.",
    "No, we have talk about this topic.",
    "We never have talked about this topic.",
    "Мы никогда не говорили на эту тему?",
    "Ми ніколи не говорили на цю тему?",
    "Czy nigdy nie rozmawialiśmy na ten temat?"
  ],
  [
    "Has he ever solved this puzzle?",
    "Yes, he has solved this puzzle.",
    "No, he have solved this puzzle.",
    "Yes, he has solving this puzzle.",
    "He has ever solved this puzzle?",
    "Он когда-нибудь решал эту головоломку?",
    "Він коли-небудь розв’язував цю головоломку?",
    "Czy on kiedykolwiek rozwiązał tę zagadkę?"
  ],
  [
    "Have you ever cooked Italian food?",
    "Yes, I have cooked Italian food.",
    "No, I have cook Italian food.",
    "Yes, I has cooked Italian food.",
    "I ever have cooked Italian food.",
    "Ты когда-нибудь готовил итальянскую еду?",
    "Ти коли-небудь готував італійську їжу?",
    "Czy kiedykolwiek gotowałeś włoskie jedzenie?"
  ],
  [
    "Have they never missed a train?",
    "No, they have never missed a train.",
    "Yes, they have miss a train.",
    "No, they never have missed a train.",
    "They has never missed a train.",
    "Они никогда не опаздывали на поезд?",
    "Вони ніколи не запізнювались на потяг?",
    "Czy oni nigdy nie spóźnili się na pociąg?"
  ],
  [
    "Has he ever climbed a mountain?",
    "Yes, he has climbed a mountain.",
    "No, he have climbed a mountain.",
    "Yes, he has climb a mountain.",
    "He has never climbed a mountain.",
    "Он когда-нибудь поднимался на гору?",
    "Він коли-небудь підіймався на гору?",
    "Czy on kiedykolwiek wspinał się na górę?"
  ],
  [
    "Have you ever written a book?",
    "Yes, I have written a book.",
    "No, I have write a book.",
    "Yes, I has written a book.",
    "I never have written a book.",
    "Ты когда-нибудь писал книгу?",
    "Ти коли-небудь писав книгу?",
    "Czy kiedykolwiek napisałeś książkę?"
  ],
  [
    "Have they ever played the piano?",
    "Yes, they have played the piano.",
    "No, they has played piano.",
    "Yes, they have playing the piano.",
    "They have ever played piano.",
    "Они когда-нибудь играли на пианино?",
    "Вони коли-небудь грали на піаніно?",
    "Czy oni kiedykolwiek grali na pianinie?"
  ],
  [
    "Has she ever won a prize?",
    "Yes, she has won a prize.",
    "No, she has win a prize.",
    "Yes, she have won a prize.",
    "She has never won a prize.",
    "Она когда-нибудь выигрывала приз?",
    "Вона коли-небудь вигравала приз?",
    "Czy ona kiedykolwiek wygrała nagrodę?"
  ],
  [
    "Have you never drawn a picture?",
    "No, I have never drawn a picture.",
    "Yes, I has never drawn a picture.",
    "No, I have draw a picture.",
    "I never have drawn a picture.",
    "Ты никогда не рисовал картину?",
    "Ти ніколи не малював картину?",
    "Czy nigdy nie rysowałeś obrazu?"
  ],
  [
    "Have they never played basketball?",
    "No, they have never played basketball.",
    "Yes, they has played basketball.",
    "No, they never have played basketball.",
    "They has never played basketball.",
    "Они никогда не играли в баскетбол?",
    "Вони ніколи не грали в баскетбол?",
    "Czy oni nigdy nie grali w koszykówkę?"
  ],
  [
    "Has he ever read a novel?",
    "No, he has never read a novel.",
    "Yes, he has reading a novel.",
    "Yes, he have read a novel.",
    "He has never read novel.",
    "Он когда-нибудь читал роман?",
    "Він коли-небудь читав роман?",
    "Czy on kiedykolwiek czytał powieść?"
  ],
  [
    "Yes, I have eaten sushi.", 
    "Have you ever eaten sushi?", 
    "Did you ever eat sushi?", 
    "Have you never eaten sushi?", 
    "Have you eat sushi?", 
    "Да, я ел суши.", 
    "Так, я їв суші.", 
    "Tak, jadłem sushi."
  ],
  [
    "No, I have never traveled by plane.", 
    "Have you ever traveled by plane?", 
    "Did you ever travel by plane?", 
    "Have you traveled by plane?", 
    "Have you never traveled by plane?", 
    "Нет, я никогда не путешествовал на самолёте.", 
    "Ні, я ніколи не подорожував літаком.", 
    "Nie, nigdy nie podróżowałem samolotem."
  ],
  [
    "Yes, she has read that book.", 
    "Has she ever read that book?", 
    "Did she ever read that book?", 
    "Has she never read that book?", 
    "Have she ever read that book?", 
    "Да, она читала эту книгу.", 
    "Так, вона читала цю книгу.", 
    "Tak, ona czytała tę książkę."
  ],
  [
    "No, I have never met a celebrity.", 
    "Have you ever met a celebrity?", 
    "Did you ever meet a celebrity?", 
    "Have you never met a celebrity?", 
    "Have you met a celebrity?", 
    "Нет, я никогда не встречал знаменитость.", 
    "Ні, я ніколи не зустрічав знаменитість.", 
    "Nie, nigdy nie spotkałem celebryty."
  ],
  [
    "Yes, they have visited New York.", 
    "Have they ever visited New York?", 
    "Did they ever visit New York?", 
    "Have they never visited New York?", 
    "Have they visit New York?", 
    "Да, они посетили Нью-Йорк.", 
    "Так, вони відвідали Нью-Йорк.", 
    "Tak, oni odwiedzili Nowy Jork."
  ],
  [
    "No, we have never gone skiing.", 
    "Have you ever gone skiing?", 
    "Did you ever go skiing?", 
    "Have you gone skiing?", 
    "Have you never gone skiing?", 
    "Нет, мы никогда не катались на лыжах.", 
    "Ні, ми ніколи не каталися на лижах.", 
    "Nie, nigdy nie jeździliśmy na nartach."
  ],
  [
    "Yes, he has cooked dinner before.", 
    "Has he ever cooked dinner?", 
    "Did he ever cook dinner?", 
    "Has he never cooked dinner?", 
    "Have he ever cooked dinner?", 
    "Да, он готовил ужин раньше.", 
    "Так, він готував вечерю раніше.", 
    "Tak, on wcześniej gotował kolację."
  ],
  [
    "No, I have never seen a shooting star.", 
    "Have you ever seen a shooting star?", 
    "Did you ever see a shooting star?", 
    "Have you seen a shooting star?", 
    "Have you never seen a shooting star?", 
    "Нет, я никогда не видел падающую звезду.", 
    "Ні, я ніколи не бачив падаючу зірку.", 
    "Nie, nigdy nie widziałem spadającej gwiazdy."
  ],
  [
    "Yes, I have danced in the rain.", 
    "Have you ever danced in the rain?", 
    "Did you ever dance in the rain?", 
    "Have you never danced in the rain?", 
    "Have you dance in the rain?", 
    "Да, я танцевал под дождём.", 
    "Так, я танцював під дощем.", 
    "Tak, tańczyłem w deszczu."
  ],
  [
    "No, she has never played the piano.", 
    "Has she ever played the piano?", 
    "Did she ever play the piano?", 
    "Has she never played the piano?", 
    "Have she ever played the piano?", 
    "Нет, она никогда не играла на пианино.", 
    "Ні, вона ніколи не грала на піаніно.", 
    "Nie, ona nigdy nie grała na pianinie."
  ],
  [
    "Yes, we have climbed a mountain.", 
    "Have you ever climbed a mountain?", 
    "Did you ever climb a mountain?", 
    "Have you never climbed a mountain?", 
    "Have you climb a mountain?", 
    "Да, мы поднимались на гору.", 
    "Так, ми піднімалися на гору.", 
    "Tak, wspięliśmy się na górę."
  ],
  [
    "No, they have never eaten sushi.", 
    "Have they ever eaten sushi?", 
    "Did they ever eat sushi?", 
    "Have they eaten sushi?", 
    "Have they never eaten sushi?", 
    "Нет, они никогда не ели суши.", 
    "Ні, вони ніколи не їли суші.", 
    "Nie, oni nigdy nie jedli sushi."
  ],
  [
    "Yes, I have watched this movie.", 
    "Have you ever watched this movie?", 
    "Did you ever watch this movie?", 
    "Have you never watched this movie?", 
    "Have you watch this movie?", 
    "Да, я смотрел этот фильм.", 
    "Так, я дивився цей фільм.", 
    "Tak, oglądałem ten film."
  ],
  [
    "No, he has never worked abroad.", 
    "Has he ever worked abroad?", 
    "Did he ever work abroad?", 
    "Has he worked abroad?", 
    "Has he never worked abroad?", 
    "Нет, он никогда не работал за границей.", 
    "Ні, він ніколи не працював за кордоном.", 
    "Nie, on nigdy nie pracował za granicą."
  ],
  [
    "Yes, they have played soccer together.", 
    "Have they ever played soccer together?", 
    "Did they ever play soccer together?", 
    "Have they never played soccer together?", 
    "Have they play soccer together?", 
    "Да, они играли в футбол вместе.", 
    "Так, вони грали у футбол разом.", 
    "Tak, oni grali razem w piłkę nożną."
  ],
  [
    "No, we have never stayed at a hotel.", 
    "Have you ever stayed at a hotel?", 
    "Did you ever stay at a hotel?", 
    "Have you stayed at a hotel?", 
    "Have you never stayed at a hotel?", 
    "Нет, мы никогда не останавливались в гостинице.", 
    "Ні, ми ніколи не зупинялися в готелі.", 
    "Nie, nigdy nie zatrzymaliśmy się w hotelu."
  ],
  [
    "Yes, she has written a book.", 
    "Has she ever written a book?", 
    "Did she ever write a book?", 
    "Has she never written a book?", 
    "Have she ever written a book?", 
    "Да, она написала книгу.", 
    "Так, вона написала книгу.", 
    "Tak, ona napisała książkę."
  ],
  [
    "No, I have never swum in the ocean.", 
    "Have you ever swum in the ocean?", 
    "Did you ever swim in the ocean?", 
    "Have you swum in the ocean?", 
    "Have you never swum in the ocean?", 
    "Нет, я никогда не плавал в океане.", 
    "Ні, я ніколи не плавав в океані.", 
    "Nie, nigdy nie pływałem w oceanie."
  ],
  [
    "Yes, we have painted a picture.", 
    "Have you ever painted a picture?", 
    "Did you ever paint a picture?", 
    "Have you never painted a picture?", 
    "Have you paint a picture?", 
    "Да, мы рисовали картину.", 
    "Так, ми малювали картину.", 
    "Tak, malowaliśmy obraz."
  ],
  [
    "No, they have never driven a car.", 
    "Have they ever driven a car?", 
    "Did they ever drive a car?", 
    "Have they driven a car?", 
    "Have they never driven a car?", 
    "Нет, они никогда не водили машину.", 
    "Ні, вони ніколи не водили машину.", 
    "Nie, oni nigdy nie prowadzili samochodu."
  ],
      // Заменяет многоточие словом
      function (id) {
         return replaceEllipsisWithWord(id, presentPerfectIHaveDone);
      }
];

    // Если это свойство есть, то оно попадает в заголовок теста
    presentPerfectIHaveDone.nameLeson = "Present Perfect";
